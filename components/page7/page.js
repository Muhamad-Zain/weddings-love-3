...
export default function Page7({ id}) {
    ...
    useEffect(() => {
        ...
        getImage()
        const messageRef = ref(database, `weddings-love-3/${id}/expresion/`)
    ...
    const handleSend = (e) =>{
        ...
        if (name === '' || message === '') {
            ...
            const messages = ref(database, `/weddings-love-3/${id}/expresion/${Date.now()}`)
            set(messages, {
                name,
                text:message
            })
            ...
            <div className={style.comment}>
                ...
                <form
                    ...
                    className='relative z-10 '>
                        ...
                        <h3 className='GVibes text-4xl text-center border-b-2 border-green-800 w-72 m-auto mb-5'>Expression</h3>
                    ...
                    <button 
                        ...
                        className='bg-[#496937] bg-opacity-80 px-4 py-2  rounded-sm text-white'
                        >Kirim</button>
                    <div className='border-b-2 border-green-800 w-full mt-2 ' />
                    <div className='h-56 overflow-y-scroll'>
                    {ucapan && ucapan.length > 0 ? (
                        ucapan.map((msg) => (
                            <p key={msg.id} className='text-green-900 border-b-2 border-green-800 py-3'>
                                <strong>{msg.name}:</strong> {msg.text}
                            </p>
                        ))
                    ) : (
                        <p className='text-green-900'>Tidak ada ucapan.</p>
                    )}
